...
- 📦 **Multiple Install Methods**: Supports `.deb` packages, tarballs, full directory tarballs, compressed binaries, direct binaries, and shell scripts
...
| `install_type` | Yes | Installation method: `deb`, `tar`, `tar_full`, `bz2`, `binary`, or `script` |
...
| `extract_dir` | For `tar_full` | Name of the extracted directory to copy to `/usr/local/` |
...
> **Note:** The script uses [PEP 723](https://peps.python.org/pep-0723/) inline script metadata and must be run with `uv run`, not a standard Python interpreter.

**Example Output:**
```
INFO: Checking bat (Current: 0.26.0)...
🚀 UPDATE FOUND for bat: 0.26.0 -> 0.26.1
...
✅ tools_versions.yml updated with versions and checksums.
```
...
- Extracts and installs full directory structures from tarballs to `/usr/local/`
...
2. Run `uv run update_versions.py` to fetch latest version and checksum

3. Deploy with `ansible-playbook deploy_tools.yml`

### For Tarballs (.tar.gz, .tar.bz2)
...
> **Note**: The playbook automatically extracts the tarball and finds the binary by name, regardless of the archive structure

### For Full Directory Tarballs (e.g., neovim)
...
> **Note**: Use `tar_full` for tools that need their complete directory structure (runtime files, libraries, etc.), not just the binary. The entire extracted directory is merged into `/usr/local/`
...
1. Add to `tools_versions.yml`:
   ...
   tools:
     fzf-git:
       version: main
       github_repo: junegunn/fzf-git.sh
       url_template: https://raw.githubusercontent.com/junegunn/fzf-git.sh/refs/heads/main/fzf-git.sh
       install_type: script
       binary_name: fzf-git.sh
   ```
...
> **Note**: Scripts are downloaded directly to `/usr/local/bin/` with executable permissions
...
```
┌─────────────────────┐
│ tools_versions.yml  │ ← Tool definitions
└──────────┬──────────┘
           │
           ├─→ update_versions.py
           │   ├─ Queries GitHub API
           ...
           └─→ deploy_tools.yml
               ...
               └─ Installs via apt or direct extraction
```
...
2. Extract entire archive to temporary directory
3. Find and copy specified binary to `/usr/local/bin`
4. Set executable permissions (`0755`)
5. Clean up temporary files

**For full directory tar archives:**
1. Download to `/tmp/<tool>.archive` with checksum verification
2. Extract to `/tmp/<extract_dir>/`
3. Use `rsync` to merge entire directory structure into `/usr/local/`
...
2. Run `uv run update_versions.py` to recalculate
...
├── tools_versions.yml      # Tool definitions and versions
├── update_versions.py      # Version checker and updater
├── deploy_tools.yml        # Ansible playbook for deployment
├── README.md              # This file
...
Currently supported: `deb`, `tar`, `tar_full`, `bz2`, `binary`, `script`

To add support for other package formats (e.g., `rpm`, `snap`):

1. Add a new task block in `deploy_tools.yml`
2. Follow the existing pattern (download → install)
